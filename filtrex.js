function compileExpression(expression) {
    if (!compileExpression.parser) {    // Building the original parser is the heaviest part. Do it
        compileExpression.parser = filtrExParser();      // once and cache the result in our own function.
    }
    var processedExpression = preProcessString(expression);
    var simpleTree = compileExpression.parser.parse(processedExpression);
    return transformTree(simpleTree);
}

function preProcessString(string) {
    var expression = " " + string.toLowerCase().replaceAll("\n", "");
    const regex = /"[^"\\]*(?:\\[\s\S][^"\\]*)*"/g;
    var bracketedKeywords = string.toLowerCase().match(regex);
    bracketedKeywords = bracketedKeywords ? bracketedKeywords : [];
    var tokens = expression.split(/ and not \(*not | or not \(*not | and \(*not | or \(*not | not | or | and |\(|\)/);
    ...
    tokens.forEach(keyword => {
        ...
        var skipKeyword = false;
        bracketedKeywords.forEach(quotedKeyword => {
            if (quotedKeyword.includes(keyword) && quotedKeyword != keyword) {
                skipKeyword = true;
            }
        });
        if (!skipKeyword) {
                expression = expression.replace('(' + keyword + ' ', '(\"' + keyword + '\" ');
                expression = expression.replace(' ' + keyword + ')', ' \"' + keyword + '\")');
                expression = expression.replace('(' + keyword + ')', '(\"' + keyword + '\")');
                expression = expression.replace(' ' + keyword + ' ', ' \"' + keyword + '\" ');
        }
...
function transformTree(tree, negate) {
    let negation = false;
    if (tree.length === 1 && typeof (tree[0]) !== "string") {   // single node go futher into tree (for double brackets)
        return this.transformTree(tree[0], negate);
    } else {
        if (tree.indexOf("!") !== -1 || negate) {   // adjust negation
            if (tree.length === 2 && negate) {
                return this.transformTree(tree[1]);                  // !a + negate
            } else if (tree.length === 2 && !negate) {
                return this.transformTree(tree[1], true);   // ! a 
            } else if (negate && typeof (tree) !== "string" && tree.length === 3) {                      // !(a or b) => !a and !b demorgans applied go futher
                tree = [tree[0], tree[1] === '&&' ? '||' : '&&', tree[2]];
                negation = true;
            }
        }
        if (typeof (tree[0]) === "string") {       // child node
            if (tree[0][0] === '"') { tree[0] = tree[0].slice(1, -1); }
            if (negate) { tree[0] = '-' + tree[0]; } // negate 
            return tree[0];
        } else {
            // tree parse futher
            if (tree[1] === "&&") {
                return { "match": 'and', "items": [this.transformTree(tree[0], negation, '&&'), this.transformTree(tree[2], negation, '&&')] };
            } else if (tree[1] === "||") {
                return { "match": 'or', "items": [this.transformTree(tree[0], negation, '||'), this.transformTree(tree[2], negation, '||')] };
            }
        }
    }
}
...
function filtrExParser() {
    ...
    var grammar = {
        ...
        bnf: {
            ...
            e: [
                ['not not e', code([3], true)],
                ['e + e', code([1, '+', 3], true)],
                ['e - e', code([1, '-', 3], true)],
                ['e * e', code([1, '*', 3], true)],
                ['e / e', code([1, '/', 3], true)],
                ['e % e', code([1, '%', 3], true)],
                ['e ^ e', code(['Math.pow(', 1, ',', 3, ')'], true)],
                ['- e', code(['-', 2], true), { prec: 'UMINUS' }],
                ['e and e', code([1, '&&', 3], true)],
                ['e or e', code([1, '||', 3], true)],
                ['e == e', code(['Number(', 1, '==', 3, ')'], true)],
                ['e != e', code(['Number(', 1, '!=', 3, ')'], true)],
                ['e ~= e', code(['RegExp(', 3, ').test(', 1, ')'], true)],
                ['e < e', code(['Number(', 1, '<', 3, ')'], true)],
                ['e <= e', code(['Number(', 1, '<=', 3, ')'], true)],
                ['e > e', code(['Number(', 1, '> ', 3, ')'], true)],
                ['e >= e', code(['Number(', 1, '>=', 3, ')'], true)],
                ['e ? e : e', code([1, '?', 3, ':', 5, true])],
                ['( e )', code([2], true)],
                ['not e', code(['!', 2], true)],
                ['NUMBER', code([1], true)],
                ['STRING', code([1], true)],
...
// ---------------------------------------------------
// Jison code appended by makefile ignore rest of the code
// ---------------------------------------------------
